...
class VoteCommonContract {
    ...
    newVote(owner, description, info) {
        ...
        const anyOption = !!info.anyOption;
    ...
    _voteFor(voteId, payer, account, option, amount) {
        ...
        if (userVotes.hasOwnProperty(option)) {
            ...
            userVotes[option] = this._clearUserVote(clearTime, [
                amount.toFixed(),
                block.number,
                '0'
            ]);
    ...
    unvote(voteId, account, option, amount) {
        ...
        if (info.deleted === FALSE) {
            freezeTime = freezeTime + info.freezeTime * 1000000000;
        }
