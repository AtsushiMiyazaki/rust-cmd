...
class VoteContract {
    ...
    logInProducer(account) {
        this._requireAuthList(this._getAccountList(account), VOTE_PERMISSION);
        if (!storage.mapHas('producerTable', account)) {
            throw new Error(_IOSTBinaryOp('producer not exists, ', account, '+'));
        }
        const pro = this._mapGet('producerTable', account);
        pro.online = true;
        this._mapPut('producerTable', account, pro, blockchain.publisher());
    ...
    _checkSwitchOff() {
        return _IOSTBinaryOp(storage.get('switchOff'), '1', '===');
    }
    switchOff(off) {
    ...
    _getWitnessWatched() {
        ...
        for (const witness in witnessWatched) {
            if (_IOSTBinaryOp(_IOSTBinaryOp(witnessWatched[witness].bn, 144 * VOTE_STAT_INTERVAL, '+'), block.number, '<')) {
    ...
    _updateWitnessPenality(witnessPenality, witnessWatched, account) {
        if (witnessWatched[account] && witnessWatched[account].count >= 3) {
            witnessPenality[account] = _IOSTBinaryOp(block.number, 144 * VOTE_STAT_INTERVAL, '+');
            delete witnessWatched[account];
        }
    ...
    stat() {
        ...
        const bn = block.number;
        if (_IOSTBinaryOp(bn % VOTE_STAT_INTERVAL, 0, '!==')) {
            return;
        }
        const voteId = this._getVoteId();
        ...
        const pendingList = newList.map(x => function(){return x.key}());
        ...
        if (_IOSTBinaryOp(bn % SCORE_DECREASE_INTERVAL, 0, '===')) {
